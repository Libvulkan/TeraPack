...
compileAll()
{
	...
	for package in $(ls "$INIT_DIR/workdir"); do
		...
		if [ -f "$INIT_DIR/built-pkgs/$package-$pkgVersion-$ARCHITECTURE.rat" ]; then
			...
			if [ ! -d "$INIT_DIR/workdir/$package/destdir-pkg/data/data/com.termux" ]; then
				echo "- Package: '"$package"' failed to compile. Check logs"
				exit 0
			fi

			cp -rf "$INIT_DIR/workdir/$package/destdir-pkg/data/data/com.termux/"* "/data/data/com.termux"
